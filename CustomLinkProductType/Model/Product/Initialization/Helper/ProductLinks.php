...
namespace AHT\CustomLinkProductType\Model\Product\Initialization\Helper;

use AHT\CustomLinkProductType\Model\Catalog\Product\Link;
...
class ProductLinks
{
    ...
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getMoutingReadonly()) {

            $links = (isset($links[self::TYPE_NAME])) ? $links[self::TYPE_NAME] : $product->getMountingLinkData();
            if (!is_array($links)) {
                $links = [];
            }

            if ($product->getMountingLinkData()) {
                $links = array_merge($links, $product->getMountingLinkData());
            }
            $newLinks = [];
